...
const DOLPHIN_PATH = path.resolve('/Users/mattdenney/Projects/slp-to-video/Ishiiruka/build/Binaries/dolphin-emu')
const SSBM_ISO_PATH = path.resolve('/Users/mattdenney/Projects/melee.iso')
const NUM_PROCESSES = 2


async function slpToVideo(combos){

    if(fs.existsSync(path.resolve('../temp'))){
        await new Promise(resolve => rimraf(path.resolve('../temp'),resolve));
    }
    fs.mkdirSync(path.resolve('../temp'));
    fs.mkdirSync(path.resolve('../temp/slp'));
    fs.mkdirSync(path.resolve('../temp/output'));
    fs.mkdirSync(path.resolve('../temp/output/set'))
    console.log("Copying files...")
    combos.forEach( (combo,index) => {
        fs.copyFileSync(
            combo.game.slpPath,
            path.resolve(`../temp/slp/temp${index}.slp`)
        );
    });
    console.log("Generating replay configs...")
    combos.forEach((combo,index) => {
        // generateReplayConfig mostly copied
        const config = {
            mode: 'normal',
        ...
        fs.writeFileSync(path.resolve(`../temp/output/set/temp${index}.json`), JSON.stringify(config))
    })
    main();
}
...
const generateReplayConfig = (file) => {
  ...
  const processReplayConfigs = async (files) => {
    console.log("Processing Replay Configs...")
  ...
  const main = () => {
    ...
    //   })
      files(path.resolve('../temp/output'))
      .then(async (files) => {
        await processReplayConfigs(files)
      })
      .then(() => subdirs(path.resolve('../temp/output')))
      .then((subdirs) => {
